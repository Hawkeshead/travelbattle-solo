...
| L15 | `toggleUnitBio` is assigned to `window` so the inline `onclick` injected via `innerHTML` can still reach it after modularization. The tidier fix is to attach the handler with `addEventListener` right after the `innerHTML` assignment and drop the global entirely. Deferred because it is a behaviour change, and the refactor's contract was to preserve behaviour exactly. | `js/ui-battle.js:355`, `:382` |
